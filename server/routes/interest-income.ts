...
const interestIncomeSchema = z.object({
  ...
  principalAmount: z.number().min(0, 'Principal amount is required'),
  interestRate: z.number().min(0, 'Interest rate is required').max(100, 'Interest rate must be between 0-100'),
  interestEarned: z.number().min(0).optional(),
