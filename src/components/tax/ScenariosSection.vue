...
// Data queries
const { data: scenarios, isLoading: scenariosLoading } = useTaxScenarios();
...
const scenariosForComparison = computed(() => {
  return scenarios.value?.filter((s: any) => selectedForComparison.value.includes(s.id)) ?? [];
});

// Create baseline from current data
async function handleCreateBaseline() {
  try {
    await createBaseline.mutateAsync(props.financialYear);
  } catch (error) {
...
<template>
  <div class="scenarios-section">
    ...
    <!-- No Baseline State -->
    <v-card v-if="!hasBaseline && !scenariosLoading" variant="outlined" class="mb-4">
      <v-card-text class="text-center py-8">
        <v-icon icon="mdi-file-document-outline" size="48" color="grey-lighten-1" class="mb-3" />
        <h4 class="text-subtitle-1 mb-2">No Baseline Scenario</h4>
    ...
    <!-- Loading State -->
    <v-skeleton-loader v-else-if="scenariosLoading" type="card, card" />
    ...
    <template v-else-if="hasBaseline">
      ...
      <v-card variant="outlined" class="mb-4 border-primary">
        <v-card-text>
          <div class="d-flex align-center mb-3">
            ...
            <span class="text-subtitle-2">Current Tax Position</span>
            <v-spacer />
            <v-btn
              variant="text"
              size="small"
          ...
          <v-row dense>
            <v-col cols="6" sm="3">
              ...
              <v-chip
                ...
                size="x-small"
              >
                {{ baseline?.selectedRegime === "NEW" ? "New" : "Old" }}
              </v-chip>
            </v-col>
            <v-col cols="6" sm="3">
            ...
            <v-col cols="6" sm="3">
              ...
              <div class="text-body-2 font-weight-bold text-currency text-error">
      ...
      <div class="mb-3 d-flex align-center">
        ...
        <div v-if="selectedForComparison.length > 0" class="text-caption text-medium-emphasis">
          {{ selectedForComparison.length }} selected (max 3)
        </div>
      </div>

      <v-row v-if="customScenarios.length > 0">
        <v-col v-for="scenario in customScenarios" :key="scenario.id" cols="12" sm="6" lg="4">
          <ScenarioCard
            :scenario="scenario"
            :baseline="baseline"
      ...
      <v-card v-else variant="outlined" class="text-center py-6">
        <v-icon icon="mdi-file-document-multiple-outline" size="40" color="grey-lighten-1" class="mb-2" />
